...
def separate_words(text, min_word_return_size):
    ...
    Chia thanh cac tu co chieu dai lon hon min_word_return_size
...
# Trích xuất các từ tốt từ list các câu và lọc theo tần suất
#
def extract_adjoined_candidates(sentence_list, stoplist, min_keywords, max_keywords, min_freq):
    ...
    for sentence in sentence_list:
        ...
        adjoined_candidates += adjoined_candidates_from_sentence(sentence, stoplist, min_keywords, max_keywords)
...
#
# Function that extracts the adjoined candidates from a single sentence - câu đơn 
#
    ...
    for num_keywords in range(min_keywords, max_keywords + 1):
        ...
        for i in range(0, len(sl) - num_keywords):
            ...
            if sl[i] not in stoplist:
                ...
                # 1) it contains at least a stopword (if it doesn't it's already been considered) chưa ít nhất 1 từ stopword
                # AND
                # 2) the last word is not a stopword - từ cuối ko phải là từ trong stopword
                # AND
...
# Lọc từ tham gia với tần số xuất hiện lớn hơn min_freq
#
def filter_adjoined_candidates(candidates, min_freq):
    # print(candidates, min_freq)
...
def generate_candidate_keyword_scores(phrase_list, word_score, min_keyword_frequency=1):
    ...
    for phrase in phrase_list:
        if min_keyword_frequency > 1:
            # print(phrase_list.count(phrase), phrase)
